...
class GardianPaper {
    ...
    public $paperBlankUrl = "https://polis.grdn.com.ua/Home/TravelConDetails";
    public $printPaperBlankUrl = "https://polis.grdn.com.ua/Home/GetPrintForm";
    public $path;
    ...
    public function getPaperOrderPage(){
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->paperBlankUrl);
        curl_setopt($ch, CURLOPT_HEADER, false);
        curl_setopt($ch, CURLOPT_REFERER, $this->siteUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    ...
    public function createPaperOrder($data){
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->saveOrderUrl);
        curl_setopt($ch, CURLOPT_HEADER, false);
        curl_setopt($ch, CURLOPT_REFERER, $this->paperBlankUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
        curl_setopt($ch, CURLOPT_SSLVERSION, 1);
...
//            return false;
    ...
    public function changeStatusPaperOrder($data, $status){
        $data['agr[Status]'] = $status;
        return $this->createPaperOrder($data);
    }

    public function printPaperBlank($pdfData){
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->printPaperBlankUrl);
        curl_setopt($ch, CURLOPT_HEADER, false);
        ...
        if($httpResponseCode == 200){
            ...
            echo $response;
            exit;
            //file_put_contents($this->path . DIRECTORY_SEPARATOR . $GUID.".pdf", $response,FILE_APPEND | LOCK_EX);
            //return true;
        }
        else {
            return false;
    ...
    public function format_phone_number( $phone ){
        ...
        $num1 = substr($phone,6,3);;
        $num2 = substr($phone,9,2);;
