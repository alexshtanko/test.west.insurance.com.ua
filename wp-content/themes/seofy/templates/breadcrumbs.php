...
*
*
* @class 		Seofy_breadcrumbs
* @version		1.0
* @category	Class
* @author 		WebGeniusLab
*/
if (!class_exists('Seofy_breadcrumbs')) {
    class Seofy_breadcrumbs {
		...
		private function text_options(){
			$this->text_home     = esc_html__('Home', 'seofy'); // text for the 'Home' link
			$this->text_category = esc_html__('Archive by Category "%s"', 'seofy'); // text for a category page
			$this->text_search   = esc_html__('Search Results for "%s" Query', 'seofy'); // text for a search results page
			$this->text_tag      = esc_html__('Posts Tagged "%s"', 'seofy'); // text for a tag page
			$this->text_author   = esc_html__('Articles Posted by %s', 'seofy'); // text for an author page
			$this->text_404      = esc_html__('Error 404', 'seofy'); // text for the 404 page
			$this->text_page     = esc_html__('Page %s', 'seofy'); // text 'Page N'
			$this->text_cpage    = esc_html__('Comment Page %s', 'seofy'); // text 'Comment Page N'		
		}
			...
			if (is_home() || is_front_page()) {
				...
				if ( is_category() ) {
					...
					if ( get_query_var('paged') ) {
						$cat = $cat->cat_ID;
						$out .= $sep . sprintf($link, get_category_link($cat), get_cat_name($cat)) . $sep . $before . sprintf($this->text_page, get_query_var('paged')) . $after;
					} else {
					...
					if ( get_post_type() != 'post' ) {
						...
						if ( get_query_var('cpage') ) {
							$out .= $sep . sprintf($link, get_permalink(), get_the_title()) . $sep . $before . sprintf($this->text_cpage, get_query_var('cpage')) . $after;
						} else {
				...
				// custom post type
					...
					$out .= sprintf($link, get_permalink($parent), $parent->post_title);
					if ($show_current) $out .= $sep . $before . get_the_title() . $after;
				...
				elseif (function_exists('is_product') && is_product()) {
	   				if(function_exists('seofy_woocommerce_breadcrumb')){
	   					$args = array();
	   					$args['delimiter'] = $sep;
	   					if ($show_home_link) $out .= $sep;
	   					$out .= seofy_woocommerce_breadcrumb($args);
	   				}
	   			}
				$out .= $wrap_after;

				echo Seofy_Theme_Helper::render_html($out);
			}
    	}
    }
    new Seofy_breadcrumbs();
}