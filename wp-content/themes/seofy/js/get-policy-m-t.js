jQuery(document).ready(function(){
	...
	/////////////////// KUTSENKO
	var AjaxUrl = location.protocol + "//" + window.location.hostname + "/plc/m/index-t.php";
	var carCat = "";
	var carNumber = "";
	var carVin = "";
	var carModelText = "";
	...
	jQuery(document).on("click", ".step-3 .btn-get-it", function(e) {
		...
		if(jQuery(this).attr('data-response-company-id') == "99999" && jQuery(this).attr('data-response-company-code') == "99999999"){
			...
			jQuery('#checkoutForm').attr('action', '/plc/m/pay_m-t.php');
		}
		...
		// else {
			...
			if(carForeign == "true" && OSAGOS[selectedOsago]["companyId"] == 8){
				...
				var daysLag = Math.ceil(Math.abs(new Date(dateTo).getTime() - new Date(dateFrom).getTime()) / (1000 * 3600 * 24));
				if(daysLag > 31){
