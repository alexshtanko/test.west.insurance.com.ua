jQuery(document).ready(function(){
	...
	function getPolicy(){
		...
		jQuery("#termStart").mask("99.99.9999", {placeholder: "дд.мм.рррр"});
	...
	/////////////////// KUTSENKO
	var AjaxUrl = location.protocol + "//" + window.location.hostname + "/plc/c/index.php";
						...
						for (var i = 0; i < request.length; i++) {
							if (request[i].id !== null) {
								...
								jQuery(".step-3-results-list").append(rowOsago + '<div class="vc_col-sm-4 vc_col-md-2">\n' +
									...
									'\t\t\t\t\t\t\t\t\t\t\t\t<div class="step-3-results-item-title">Ціна</div><span class="price" ' + allPrices + allPricesDcv + '>' + selectedPrice + '</span> <span class="currency">грн</span>\n' +
									'\t\t\t\t\t\t\t\t\t\t\t</div>\n' +
									'\t\t\t\t\t\t\t\t\t\t</div>\n' +
	...
	jQuery(document).on("click", ".step-3 input:radio", function() {
		...
		jQuery(this).parents(".step-3-results-item-top").find(".step-3-price span.price").html(parseFloat(tempSelectedFranchise) + parseFloat(tempSelectedDcv));
	});
	...
	jQuery(document).on("click", ".step-3 .btn-get-it", function(e) {
		...
		jQuery('#checkoutForm').attr('action', '/plc/c/pay.php');
	...
	jQuery('.step-4 .btn-get-it').on('click', function() {
		jQuery('form#checkoutForm').attr({"action":"/plc/c/pay.php", "target":"_self"});
		checkForm();
	});
			...
			jQuery.ajax({
				...
				success: function (data) {
					...
					if (request.status == "OK") {
						...
						window.location = '/plc/c/payment.php?code=' + request.code;
						//}
					} else {
						alert(request.data);
					}
