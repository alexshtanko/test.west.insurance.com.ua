...
class Insurance_Activator {
	...
	public static function activate() {
		...
		$activator = new Insurance_Activator();
        ...
        //Создание таблички для СК Евроинс
        $activator->create_table_insurance_euroins_orders();

        //Создание таблички для СК EKTA
        $activator->create_table_insurance_ekta_orders();


        // Создание таблички Gardian
        $activator->create_table_insurance_gardian_orders();

        /*
         * Запустить єти методы один раз для после реализации нового функционала
         * "Електронные бланки"
         * Добавляет поля в таблички для новых типов бланков "Электронные" blank_type_id
         * */

//        $activator->updateTableInsuranceRate();
//        $activator->updateTableInsuranceOrders();
...
//        $activator->create_table_insurance_e_blank_number_list();
//        $activator->blnak_number_to_insurance_e_blank_number_list();
...
//        $activator->updateUserInsuranceVisibleCompanies();
	...
	public function createTableOrders() {
		...
		$sql = "CREATE TABLE {$table_name} (
			...
			`rate_locations` varchar(50) NOT NULL,
			...
			`eddr` varchar(14) NOT NULL default '',
	...
	public function createBlankToManager() {
		...
		$table_name = $wpdb->get_blog_prefix() . 'insurance_blank_to_manager';
    ...
    /*
     * Добавить столбец в табилцу "Тарифы" insurance_rate
     * */
    public function updateTableInsuranceRate(){
    ...
    /*
     * Добавить столбец в табилцу "Тарифы" insurance_orders
     * */
    public function updateTableInsuranceOrders(){
     ...
     *
     * */
    public function create_table_insurance_e_blank_number_list()
    {
    ...
    /*
     * Создание таблички для СК ЕВРОИНС
     * Нужно для сохранения ИД возвращаемого после оформления договоров по API
     *
     * */
    public function create_table_insurance_euroins_orders()
    {
        ...
        $sql = "CREATE TABLE {$table_name} (
			...
			`insuranceApplicationID` varchar(255) NOT NULL default '',
			`polisNumber` varchar(255) NOT NULL default '',
			`status` int(1) unsigned NOT NULL default '0',
			`date_added` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			`date_change` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			PRIMARY KEY  (id)			
     ...
     *
     * */
    public function create_table_insurance_ekta_orders()
    {
        ...
        $sql = "CREATE TABLE {$table_name} (
			`id` int(10) unsigned NOT NULL auto_increment,
			`ekta_id` INT(12) NOT NULL,
			`ekta_order_id` INT(12) NOT NULL,
			`ekta_cost` decimal(12,2) NOT NULL,
			`order_id` INT(12) NOT NULL,
			`status` int(1) unsigned NOT NULL default '0',
			`date_added` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			`date_change` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    /*
     * Создание таблички для СК GARDIAN
     * Нужно для сохранения ИД возвращаемого после оформления договоров по API
     *
     * */
    public function create_table_insurance_gardian_orders()
    {
        ...
        $sql = "CREATE TABLE {$table_name} (
			`id` int(10) unsigned NOT NULL auto_increment,
			`gardian_GUID` varchar(100) NOT NULL default '',
			`gardian_CustomerGUID` varchar(100) NOT NULL default '',
			`gardian_Number` varchar(100) NOT NULL default '',
			`gardian_ObjectGUID` varchar(100) NOT NULL default '',
			`order_id` INT(12) NOT NULL,
			`status` int(1) unsigned NOT NULL default '0',
			`date_added` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
			`date_change` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
     ...
     *
     * */
    private function blnak_number_to_insurance_e_blank_number_list()
    {
        ...
        foreach ( $e_blank_diapason as $blank_diapason )
        {
            ...
            {
                $this->set_blnak_number_to_insurance_e_blank_number_list($i, $blank_diapason['id']);
                $i++;
            }
    ...
    private function change_staus_e_blank_number_list()
    {
        ...
        $orders = $this->get_order_by_blank_type(2);
        ...
        foreach( $orders as $order ){
            ...
            $wpdb->update( $table_name,
                [ 'status' => $order['status'], 'date_change' => $date_time->getTimestamp() ],
                [ 'blank_number' => $order['blank_number'], 'number_of_blank_id' => $order['number_blank_id'] ],
                [ '%d', '%s' ],
                [ '%d', '%d' ]
            );
    ...
    /*
     * Получить все нумерации ЭЛЕКТРОННЫХ бланков с таблички insurance_e_blank_number_list
     *
     * return ARRAY
     * */
     ...
     */
    public  function updateUserInsuranceVisibleCompanies( $role = 'user_manager' )
    {
        $arg = [
           'role__in' => [$role],
        ];
        ...
        $companies = $this->get_insurance_companies( 1 );
            ...
            foreach ( $users as $user ) {
                if ($companies) {
                    foreach ($companies as $company) {
                        update_user_meta( $user->ID, 'user_insurance_company_visible_status_' . $company['id'], 1);
                    }
                }
            }
        }
     ...
     * return ARRAY
     */
    public function get_insurance_companies( $table = 1 )
    {
        global $wpdb;
