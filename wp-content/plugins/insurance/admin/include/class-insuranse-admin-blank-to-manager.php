...
class Insurance_Admin_Blank_To_Manager
{
...
//		add_action('admin_head', array($this, 'insurance_add_number_of_blank_to_manager_js'));
        add_action( 'admin_print_scripts-medical-policy_page_insurance-blank-to-manager', array( $this, 'insurance_add_number_of_blank_to_manager_js' ) );
...
//		wp_enqueue_script('insuranceadminaddblanktomanager', PLUGIN_URL . 'admin/js/insurance-admin-add-blank_to_manager.js', array('jquery'), $this->version);
	...
	public function insurance_add_blank_to_manager($postData, $managerId)
	{
		...
		if (current_user_can('medical_insurance_number_of_blanks')) {
			...
			$blanks_table = $wpdb->get_blog_prefix() . 'insurance_number_of_blank';
			$blank_to_manager_table = $wpdb->get_blog_prefix() . 'insurance_blank_to_manager';
	...
	public function get_blanks_to_manager($managerId)
	{
		...
		$table_blanks = $wpdb->get_blog_prefix() . 'insurance_number_of_blank';
		$table_company = $wpdb->get_blog_prefix() . 'insurance_company';
	...
	public function insurance_delete_blank_to_manager($postData)
	{
