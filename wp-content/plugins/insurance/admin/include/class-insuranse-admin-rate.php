...
class Insurance_Admin_Rate {
    ...
    public function __construct( $plugin_name, $version ) {
        ...
        */
//        add_action( 'admin_head', array( $this, 'insurance_rate_list_js' ) );
        add_action( 'admin_print_scripts-medical-policy_page_insurance-rate', array( $this, 'insurance_rate_list_js' ) );

		//Экшн Вывода тарифов 
		add_action( 'wp_ajax_insuranceratelist', array( $this, 'insurance_rate_list' ) );
        add_action( 'wp_ajax_nopriv_insuranceratelist', array( $this, 'insurance_rate_list' ) );

        /*
        * Удалить тарий
        */
//        add_action( 'admin_head', array( $this, 'insurance_rate_delete_js' ) );
        add_action( 'admin_print_scripts-medical-policy_page_insurance-rate', array( $this, 'insurance_rate_delete_js' ) );

		//Экшн удаления тарифа 
		add_action( 'wp_ajax_insuranceratedelete', array( $this, 'insurance_rate_delete' ) );
        add_action( 'wp_ajax_nopriv_insuranceratedelete', array( $this, 'insurance_rate_delete' ) );

    }

    public function insurance_rate_delete_js() {

//		wp_enqueue_script( 'insuranceadminratedelete', PLUGIN_URL . 'admin/js/insurance-admin-rate-delete.js', array('jquery'), $this->version );
		wp_enqueue_script( 'insuranceadminratedelete', plugins_url( 'js/insurance-admin-rate-delete.js', dirname(__FILE__) ), array('jquery'), $this->version );

		wp_localize_script( 'insuranceadminratedelete', 'insuranceAdminRateDelete', array(
			'ajaxurl' => admin_url('admin-ajax.php'),
			'nonce' => wp_create_nonce( 'insurance-admin-rate-delete' ) // Create nonce which we later will use to verify AJAX request
        ));
    ...
    //Удалить бланк
	public function insurance_rate_delete() {

		if( empty( $_POST['nonce'] ) ){
			wp_die('', '', 400);
		}
	
		check_ajax_referer( 'insurance-admin-rate-delete', 'nonce', true );

		if( current_user_can('medical_insurance_import') ){
            ...
            $rates_sort = $this->rates_sort( $company_title, $blank_title, $franchise, $validity, $sum, $location );
            ...
            $rates_sort = $this->rates_sort( $company_title, $blank_title, $franchise, $validity, $sum, $location, $rates_limit_from, $count );
    ...
    public function insurance_rate_list_js() {

//		wp_enqueue_script( 'insuranceadminratelist', PLUGIN_URL . 'admin/js/insurance-admin-rate-list.js', array('jquery'), $this->version );
		wp_enqueue_script( 'insuranceadminratelist', plugins_url( 'js/insurance-admin-rate-list.js', dirname(__FILE__) ), array('jquery'), $this->version );

		wp_localize_script( 'insuranceadminratelist', 'insuranceAdminRateList', array(
			'ajaxurl' => admin_url('admin-ajax.php'),
			'nonce' => wp_create_nonce( 'insurance-admin-rate-list' ) // Create nonce which we later will use to verify AJAX request
        ));
	...
	public function insurance_rate_list() {

		if( empty( $_POST['nonce'] ) ){
			wp_die('', '', 400);
		}
	
		check_ajax_referer( 'insurance-admin-rate-list', 'nonce', true );
    ...
    public function import() {
        ...
        add_action('admin_post_nopriv_insurance_admin_rate_import', array( $this, 'insurance_rate_import' ));
        add_action('admin_post_insurance_admin_rate_import', array( $this, 'insurance_rate_import' ));

    }

    public function insurance_rate_import_js() {

//		wp_enqueue_script( 'insuranceadminrateimport', PLUGIN_URL . 'admin/js/insurance-admin-rate-import.js', array('jquery'), $this->version );
		wp_enqueue_script( 'insuranceadminrateimport', plugins_url( 'js/insurance-admin-rate-import.js', dirname(__FILE__) ), array('jquery'), $this->version );

		wp_localize_script( 'insuranceadminrateimport', 'insuranceAdminRateImport', array(
			'ajaxurl' => admin_url('admin-ajax.php'),
			'nonce' => wp_create_nonce( 'insurance-admin-rate-import' ) // Create nonce which we later will use to verify AJAX request
        ));
        

    }
    
    public function insurance_rate_upload_db ( $rows, $balnk_type_id = 1 ){
        ...
        if( current_user_can('medical_insurance_import') ){
            ...
            $table_name = $wpdb->get_blog_prefix() . 'insurance_rate';
    ...
    public function insurance_rate_update_db ( $rows, $blank_type_id = 1 ){
        ...
        if( current_user_can('medical_insurance_import') ){
            ...
            foreach( $rows as $row ){
                ...
                $res = $wpdb->update( $table_name, array( 'price' => $row['price'] ), array( 'id' => $row['id'], 'blank_type_id' => $blank_type_id ) );
	...
	public function insurance_rate_import() {
        ...
        if( wp_verify_nonce( $_POST['fileup_nonce'], 'form_rate_file_upload' ) ){
            ...
            if ( $movefile && empty($movefile['error']) ) {
                ...
                foreach( $tmp_results as $tmp_result ){
                    ...
                    array_splice( $tmp_result, 4, 2 );
                    $excell_array[] = implode( '', $tmp_result );
                }

                //Получаем все Бланки с БД
                $all_rates_from_db = $this->get_all_rates_for_upload( $company_id, $program_id, $blank_type_id );
                ...
                $result = $this->insurance_rate_upload_db( $result_upload, $blank_type_id );
                

                //Обновляем "Цену" Тарифа в БД
                $this->insurance_rate_update_db( $result_update, $blank_type_id );
        ...
        // $message[] = "Файли успiшно завантаженi!\n";
    ...
    */
    public function get_rates( $limit_from = 0, $offset = 99999999) {
		...
		$results = $wpdb->get_results( $wpdb->prepare("SELECT ir.id, ir.franchise, ir.validity, ir.insured_sum, ir.price, ir.locations, ic.title as commpany_title, ib.title as blank_title 
    ...
    *   return ARRAY 
    */
    public function rates_sort( $company_title, $blank_title, $franchise, $validity, $sum, $location, $rates_limit_from = 0, $offset = 999999 ) {
		...
		$results = $wpdb->get_results( "SELECT ir.id, ir.franchise, ir.validity, ir.insured_sum, ir.price, ir.locations, ic.title as commpany_title, ib.title as blank_title 
        FROM ". $table_name_rate ." ir 
        left join " . $table_name_company . " ic on ic.id = ir.company_id 
    ...
    *   return ARRAY 
    */
    public function get_all_rates_for_upload( $company_id, $program_id, $blank_type_id = 1 ) {
        // public function get_rates() {
        ...
        FROM " . $table_name_rate . " WHERE company_id = " . (int)$company_id . " AND program_id = " . (int)$program_id . " AND blank_type_id = " . (int)$blank_type_id . " ORDER BY id ASC", '%d', '%d' ), ARRAY_A );
