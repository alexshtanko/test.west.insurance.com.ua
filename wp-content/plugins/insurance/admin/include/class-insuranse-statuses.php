...
class Insurance_Admin_Statuses {
...
//		add_action( 'admin_head', array( $this, 'insurance_add_statuses_js' ) );
        add_action( 'admin_print_scripts-medical-policy_page_insurance-statuses', array( $this, 'insurance_add_statuses_js' ) );
	}

	public function insurance_add_statuses_js() {
//		wp_enqueue_script( 'insuranceadminaddstatuses', PLUGIN_URL . 'admin/js/insurance-admin-statuses.js', array('jquery'), $this->version );
		wp_enqueue_script( 'insuranceadminaddstatuses', plugins_url( 'js/insurance-admin-statuses.js', dirname(__FILE__) ), array('jquery'), $this->version );
	}

	public function insurance_add_status($postData){
		...
		if( current_user_can('create_users') ){
			...
			$statuses_table = $wpdb->get_blog_prefix() . 'insurance_statuses';
	...
	public function insurance_edit_status($postData){
		// проверим возможность
		if( current_user_can('create_users') ){
			global $wpdb;
	...
	public function render_statuses_list( $statuses ) {
		...
		if( count($statuses) > 0 ){
			...
			foreach( $statuses as $status ){
				...
				$edit = '<a href="' . admin_url( 'admin.php?page=insurance-statuses&getStatusId='.$id ) . '" class="button button-primary button-large ">
	...
	public function insurance_delete_status($postData){
