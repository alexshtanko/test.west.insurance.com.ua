    ...
    <h1><?php echo get_admin_page_title(); if(isset($currentUser)) echo " (".$currentUser->display_name.")"?></h1>
        <?php
            if(array_key_exists("managerid", $_GET)) {
                echo '<p><a href="' . admin_url( 'admin.php?page=insurance-blank-to-manager' ) . '" class="button button-primary button-large">
...
if(array_key_exists("managerid", $_GET)){
    ...
    <form action="" method="POST">
        ...
        <div style="display: inline-block; vertical-align:top">
            <label>&nbsp;</label> <br/>
            <input type="hidden" name="filter" value="insurance_download_xlsx"/>
	...
	<table class="wp-list-table widefat fixed striped posts">
		...
		<?php
			...
			foreach($managers as $key => $value){
				echo '<tr>
						<td>'.$counter.'</td>
						<td><a href="'.admin_url('admin.php?page=insurance-blank-to-manager&managerid='.$key).'">'.$value.'</a></td>
						<td class="text-center manage-column">
						<a href="'.admin_url('admin.php?page=insurance-blank-to-manager&managerid='.$key).'" class="button button-primary button-large js-remove-btn"><i class="fa fa-edit"></i></a>
						</td>';
				$counter++;
			}
		?>
