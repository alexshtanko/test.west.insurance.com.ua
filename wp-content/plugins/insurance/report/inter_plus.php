...
if ( $key == $_GET['key'] ){
	...
	$excludeGardian = ' AND company_id = 5 ';
...
(DATE_FORMAT(CURRENT_DATE, '%m%d') < DATE_FORMAT(birthday, '%m%d')) /* step 2 */
) AS years, passport, address, CONCAT( `blank_series`, `blank_number` ) as contractNumber, company_title, date_from as dateFrom, date_to as dateTo, count_days as countDays, rate_locations as country, rate_insured_sum as sumInsured, rate_price as insuranceRate, rate_franchise as franchise, rate_coefficient, date_added, program_title, number_blank_comment as numberBlankComment FROM " . $table_name . " WHERE date_added >= (CURRENT_DATE() - 1) AND status = 1 ".$excludeGardian." ORDER BY id DESC;", ARRAY_A );
	...
	if(count($excelData) > 0){
		...
		if( $status ){
			...
			$comment = 'Inter plus | Сформовано та вiдправлено ' . $orders_count;
