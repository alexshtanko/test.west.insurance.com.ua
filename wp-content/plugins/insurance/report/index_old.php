...
if ( $key == $_GET['key'] ){
	...
	if($selectedDate == "") {
		...
		$excludeGardian = ' AND company_id <> 2 ';
	}
	...
	// ) AS years, passport, address, phone_number as contractNumber, date_from as dateFrom, date_to as dateTo, count_days as countDays, rate_locations as country, rate_insured_sum as sumInsured, rate_price as insuranceRate, rate_franchise as franchise FROM plc_insurance_orders WHERE date_added >= CURRENT_DATE() AND status = 1 ORDER BY id DESC;", ARRAY_A );

	$data = $wpdb->get_results( "SELECT name, last_name as firstName, birthday,
(
(YEAR(CURRENT_DATE) - YEAR(birthday)) -                             /* step 1 */
(DATE_FORMAT(CURRENT_DATE, '%m%d') < DATE_FORMAT(birthday, '%m%d')) /* step 2 */
) AS years, passport, address, CONCAT( `blank_series`, `blank_number` ) as contractNumber, company_title, date_from as dateFrom, date_to as dateTo, count_days as countDays, rate_locations as country, rate_insured_sum as sumInsured, rate_price as insuranceRate, rate_franchise as franchise, rate_coefficient, date_added, number_blank_comment as numberBlankComment FROM " . $table_name . " WHERE date_added >= CURRENT_DATE() AND status = 1 ".$excludeGardian." ORDER BY id DESC;", ARRAY_A );

	if( $selectedDate != "" ) {
		$dataSecond = $wpdb->get_results( "SELECT name, last_name as firstName, birthday,
(
(YEAR(CURRENT_DATE) - YEAR(birthday)) -                             /* step 1 */
(DATE_FORMAT(CURRENT_DATE, '%m%d') < DATE_FORMAT(birthday, '%m%d')) /* step 2 */
) AS years, passport, address, CONCAT( `blank_series`, `blank_number` ) as contractNumber, company_title, date_from as dateFrom, date_to as dateTo, count_days as countDays, rate_locations as country, rate_insured_sum as sumInsured, rate_price as insuranceRate, rate_franchise as franchise, rate_coefficient, date_added, number_blank_comment as numberBlankComment FROM " . $table_name . " WHERE date_added >= '".$selectedDate."' AND status = 1 AND company_id = 2 AND id > 547 ORDER BY id DESC;", ARRAY_A );

		$data = array_merge($data, $dataSecond);
	}
