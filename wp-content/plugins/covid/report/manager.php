...
require_once $_SERVER['DOCUMENT_ROOT'].'/wp-content/plugins/covid/admin/include/class-covid-admin-help.php';
...
//	}
    ...
    $table_name_covid_blank_type = $wpdb->get_blog_prefix() . 'covid_blank_type';
	$blank_types_data = $wpdb->get_results( "SELECT id, text FROM ".$table_name_covid_blank_type." WHERE `status` = 1", ARRAY_A);
	...
	if ( count( $data ) > 0 ) {
		...
		foreach ( $data as $row ) {
			...
			if($row['status'] == 1) {
                ...
                $insurers = $insurance->get_covid_insurers( $order_id );
