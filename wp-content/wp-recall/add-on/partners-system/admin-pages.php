...
function options_ps(){
	...
	$key = rcl_key_addon(pathinfo(__FILE__));
        if(!isset($ps_options['partners_level'])) $ps_options['partners_level'] = 2;
        //print_r($ps_options);
	$content .= '
	<div id="options-'.$key.'" class="wrap-recall-options" style="display:block;">
		<div class="option-block">
                    ...
                    <input type="number" name="default_partner" value="'.$ps_options['default_partner'].'">
                    <input type="hidden" name="default_partner_on" value="0">
                    <input type="checkbox" name="default_partner_on" '.checked(1,$ps_options['default_partner_on'],false).' value="1"> '.__('Included','partners-system').'
                    ...
                    <input type="checkbox" name="transfer_referals" '.checked(1,$ps_options['transfer_referals'],false).' value="1"> '.__('Included','partners-system').'<br>
                    ...
                    <input type="checkbox" name="link_partner" '.checked(1,$ps_options['link_partner'],false).' value="1"> '.__('Included','partners-system').'<br>
                    ...
                    <input type="number" name="refers_amount" value="'.$ps_options['refers_amount'].'"><br>
                    ...
                    <input type="checkbox" name="promocode" '.checked(1,$ps_options['promocode'],false).' value="1"> '.__('Included','partners-system').'<br>
                    ...
                    <input type="number" name="return_amount" value="'.$ps_options['return_amount'].'">%<br>
                    ...
                    for($a=1;$a<=50;$a++){
                        $content .= '<option value="'.$a.'" '.selected($a,$ps_options['partners_level'],false).'>'.$a.'</option>';
                    }
                    $content .= '</select>';
		$content .= '</div>
		...
		<div class="option-block">';
                    ...
                    for($b=1;$b<=100;$b++){
                        ...
                        foreach($actions as $key=>$action){
                         ...
                         $content .= '</div>';
                         if($ps_options['partners_level']==$b) $stl = 'style=display:none;"';
                    }
                    $content .= '
		</div>
	</div>';
...
function update_options_ps ( ) {
  if ( isset( $_POST['options-ps'] ) ) {
	if( !wp_verify_nonce( $_POST['_wpnonce'], 'update-options-ps' ) ) return false;
        if(get_option(WP_PREFIX.'prtsys')!=WP_HOST) return false;
        $lvl = $_POST['partners_level'];
        foreach($_POST as $key => $value){
		if($key=='options-ps') continue;
                if($key=='levels'){
...
add_action('init', 'update_options_ps');
...
function admin_incentives_list(){
	...
	foreach($incentives as $incentive){
		...
		switch($incentive->type_inc){
			case 1: $type_inc = __('Reputation','partners-system'); break;
			case 2: $type_inc = __('Lump sum payment','partners-system'); break;
			case 3: $type_inc = __('Процент (платеж)','partners-system'); break;
                        case 4: $type_inc = __('Процент (баллы)','partners-system'); break;
		}
                
                switch($incentive->type_inc){
                    case 1: $end = ' '.__('points','partners-system'); break;
                    case 4: $end = ' '.__('points','partners-system'); break;
		...
		$table .= '<tr';
                if($n%2==0) $table .= ' class="alternate"';
                $table .= '>
...
function admin_partners_list(){
	global $wpdb;
		...
		if(isset($_GET['paged'])) $page = $_GET['paged'];
		else $page=1;
        ...
        if($referals){
            ...
            foreach($referals as $referal){
                    ...
                    $table .= '>'
                    . '<td>'.++$n.'</td>'
                    . '<td><a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=partners-list&partner='.$referal->partner.'">'.get_the_author_meta('display_name',$referal->partner).'</a></td>'
