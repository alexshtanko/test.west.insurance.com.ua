...
//        $referal = $wpdb->get_results( "SELECT referal FROM " . WP_PREFIX . "prt_partners WHERE partner='$user_id'", ARRAY_A );
    ...
    */
    public function render_filter_form( $data ) {
        ...
        $result .= '<button class="js-covid-order-filter-btn">Фільтрувати</button>';
    ...
    /*  Получаем COVID заявки
    *   return array
    */
    public function get_covid_orders( $user_id, $limit_from = 0, $offset = 99999999 ) {
    ...
    /*  Получаем COVID заявку
    *   return array
    */
    public function get_covid_order( $order_id ) {
    ...
    */
    public function pagination( $page = 1, $orders_count = 1, $pages = 1 ) {
        ...
        $result .= '<div class="add-on-insurance-pagination-btn add-on-covid-pagination-btn-prev"><i class="fa fa-angle-left" aria-hidden="true"></i></div>';
        $result .= '<input class="add-on-insurance-pagination-inpt js-add-on-covid-pagination-inpt" type="text" value="' . $page . '">';
        $result .= '<div class="add-on-insurance-pagination-btn add-on-covid-pagination-btn-next"><i class="fa fa-angle-right" aria-hidden="true"></i></div>';

        return $result;

    }

    /*  Отрисовываем COVID заявки
    *   return html
    */
    public function render_covid_orders( $orders ) {
        ...
        foreach( $orders as $order ){
            ...
            if( $insurer_status ){
                ...
                //Страхувальники
                $insurers = $this->get_covid_insurers( $order['id'] );
            ...
            $order_blank_btn = "<a target='_blank' class='button button-primary button-large more-data get-order-excell' href='/wp-content/plugins/covid/order-print/electronic-form/electronic-form.php?order_id=" . $order['id'] . "&key=TdHjjZycfXfqRF7Ydao4'><i class='fa fa-file-excel-o' aria-hidden='true'></i></a>";
            ...
            $result .= "<tr>
					...
					<td>".$date_added."</td>
					<td>
                        <button type='submit' class='button button-primary button-large more-data js-get-covid-order-info' data-order-id='".$order['id']."''><i class='fa fa-eye'></i></button>" . $order_blank_btn . "
                        
                        <button class='button button-primary button-large more-data js-covid-removal-request' data-blank-number='". $order['blank_number'] ."' data-blank-series='". $order['blank_series'] ."' data-order-id='".$order['id']."' data-unicue-code='" . $order['unicue_code'] . "'><i class='fa fa-trash-o' aria-hidden='true'></i></button>               
                    </td>
				  </tr>";
    ...
    /*  Отрисовываем COVID заявку
    *   return html
    */
    public function render_covid_order( $order ) {
    ...
    */
    public function export_excell( $order_id ) {
        ...
        if( array_intersect($allowed_roles, $user->roles ) ) {
            ...
            $orders = $this->get_covid_orders( $get_all_referals_id );
     ...
     * return array
     */
    public function get_covid_order_statuses(){
    ...
    public function render_statuses( $statuses, $order_id, $unicue_code = '', $blank_number, $blank_series ){
        ...
        $result .='<button class="button button-primary button-large box-center" id="covidSendRemovalRequest" data-blank-number="'. $blank_number .'" data-blank-series="'. $blank_series .'" data-order-id="'. $order_id .'" data-unicue-code="' .  $unicue_code .'">Надіслати запит</button>';
...
//        wp_die();
    ...
    //Получаем всех COVID страховальников
    //return array

    public function get_covid_insurers( $order_id ){
