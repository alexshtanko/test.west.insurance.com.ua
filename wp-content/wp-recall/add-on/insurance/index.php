...
add_action('init','add_tab_insurance');

//Показать подробную информацию по заказу  
if (!is_admin()):
    add_action('rcl_enqueue_scripts','add_on_order_info_js',10);
endif;

if(defined('DOING_AJAX') && DOING_AJAX){
    add_action('wp_ajax_nopriv_order_info', 'add_on_order_info'); // No privileges - т.е. гость
    add_action('wp_ajax_order_info', 'add_on_order_info'); // любой залогиненный
}

function add_on_order_info_js(){
    rcl_enqueue_style('add-on-insurance-style',rcl_addon_url('css/style.css', __FILE__));
    rcl_enqueue_script( 'add-on-insurance-order-info', rcl_addon_url('js/add-on-insurance-order-info.js', __FILE__) );
}


function add_on_order_info() {
    ...
    $order_data = $insurance->get_order( $order_id );

    $render_order = $insurance->render_order( $order_data );
...
if(defined('DOING_AJAX') && DOING_AJAX){
    add_action('wp_ajax_nopriv_removal_request', 'add_on_removal_request'); // No privileges - т.е. гость
    add_action('wp_ajax_removal_request', 'add_on_removal_request'); // любой залогиненный
}

function add_on_removal_request() {
    ...
    $statuses = $insurance->get_order_statuses();
...
//Отправка Email
if(defined('DOING_AJAX') && DOING_AJAX){
    add_action('wp_ajax_nopriv_send_email', 'add_on_send_email'); // No privileges - т.е. гость
    add_action('wp_ajax_send_email', 'add_on_send_email'); // любой залогиненный
}


function add_on_send_email() {
    ...
    $insurance_class = new Insurance_Class();

    $order_data = $insurance_class->get_order( $order_id );
    $company_id = $order_data['company_id'];
    $blank_type = $order_data['blank_type_id'];

    $order_date_added = strtotime($order_data['date_added']);
...
//            $message = '<p>Користувач '. $user_last_name . ' '. $user_name .' подає запит на змiну статуса на <b>' . $status_text . '</b> у полiса Серія: ' . $blank_series . ' № '. $blank_number .' з ID = ' . $order_id . '</p><a href="http://epolicy.com.ua/wp-content/plugins/insurance/change/change_status.php?key='. $key .'&order_id='. $order_id .'&status='. $status .'&unicue_code='. $unicue_code .'">Змiнити статус.</a><p>Коментар: '. $comment .'</p>';

            if( $company_id == 2 && $order_id >= 22201 )
            {
//                $gardian = new GardianPaper(__DIR__);
                $gardian = new Gardian(__DIR__);

                $order_data = $gardian->get_gardian_order( $order_id );
                $message = '<p>Користувач '. $user_last_name . ' '. $user_name .' подає запит на змiну статуса на <b>' . $status_text . '</b> у полiса Серія: ' . $blank_series . ' № '. $blank_number .' з ID = ' . $order_id . '</p><a href="'.get_site_url().'/wp-content/plugins/insurance/change/change_status.php?key='. $key .'&order_id='. $order_id .'&status='. $status .'&unicue_code='. $unicue_code .'">Змiнити статус (в системi Epolicy).</a><p><a target="_blank" href="https://polis.grdn.com.ua/Home/Login">Для змiни статусу договору зайдiть в особитий кабiнет СК ГАРДIАН</a> № договору '. $order_data['gardian_Number'].'</p><p>Коментар: '. $comment .'</p>';
            }
            elseif( $company_id == 4 )
            {
                $message = '<p>Користувач '. $user_last_name . ' '. $user_name .' подає запит на змiну статуса на <b>' . $status_text . '</b> у полiса Серія: ' . $blank_series . ' № '. $blank_number .' з ID = ' . $order_id . '</p><a href="'.get_site_url().'/wp-content/plugins/insurance/change/change_status.php?key='. $key .'&order_id='. $order_id .'&status='. $status .'&unicue_code='. $unicue_code .'">Змiнити статус (в системi Epolicy).</a><p>Коментар: '. $comment .'</p>';
            }
            elseif( $company_id == 6 )
            {
                ...
                $ekta_id = $ekta_order_data['ekta_id'];
                $subject = 'Epolicy. Запит на змiну статуса в СК ЕКТА.';
                $message = '<p>Користувач '. $user_last_name . ' '. $user_name .' подає запит на змiну статуса на <b>' . $status_text . '</b> у полiса Серія: ' . $blank_series . ' № '. $blank_number .' з ID = ' . $order_id . '</p><a href="'.get_site_url().'/wp-content/plugins/insurance/change/change_status.php?key='. $key .'&order_id='. $order_id .'&status='. $status .'&unicue_code='. $unicue_code .'">Змiнити статус.</a><p>ID для змiни статуса у СК ЕКТА - ' . $ekta_id . '</p><p>Коментар: '. $comment .'</p>';
            }
            else
            {
                $message = '<p>Користувач '. $user_last_name . ' '. $user_name .' подає запит на змiну статуса на <b>' . $status_text . '</b> у полiса Серія: ' . $blank_series . ' № '. $blank_number .' з ID = ' . $order_id . '</p><a href="'.get_site_url().'/wp-content/plugins/insurance/change/change_status.php?key='. $key .'&order_id='. $order_id .'&status='. $status .'&unicue_code='. $unicue_code .'">Змiнити статус.</a><p>Коментар: '. $comment .'</p>';
...
//            $message = '<p>Користувач '. $user_last_name . ' '. $user_name .' подає запит на змiну статуса на <b>' . $status_text . '</b> у полiса Серія: ' . $blank_series . ' № '. $blank_number .' з ID = ' . $order_id . '</p><a href="'.get_site_url().'/wp-content/plugins/insurance/change/change_status.php?key='. $key .'&order_id='. $order_id .'&status='. $status .'&unicue_code='. $unicue_code .'">Змiнити статус.</a><p>Коментар: '. $comment .'</p>';
            ...
            $insurance->send_email( $to, $subject, $message );
        }
...
//                    $gardian = new GardianPaper(__DIR__);
                    ...
                    if($gardian->loginPage()) {
                        if ($gardian->login() == 200) {
                            ...
                            $gardian_data = $gardian->create_array_data( $order_id, 'Draft' );

//                            $gardian->changeStatusPaperOrder($gardian_data, "Draft");
                            $gardian->changeOrderStatus($gardian_data, "Draft");
                            $gardian->change_order_status( $order_id, $status );
                            $insurance_class->change_status($order_id, $status, $unicue_code);
                ...
                elseif ($company_id == 6) {
                    ...
                    $message = '<p>Користувач ' . $user_last_name . ' ' . $user_name . ' подає запит на змiну статуса на <b>' . $status_text . '</b> у СК ЕКТА. ID для змiни статуса у СК ЕКТА - ' . $ekta_id . '</p>';
        ...
        elseif( $blank_type == 2 )
        {
            ...
            // Изменяем статус в нумерации электронных бланков
            $insurance_class->set_status_e_blank( $order_data['number_blank_id'], $order_data['blank_number'], 0 );
//            $insurance_class->set_status_e_blank( 138, 423896, 0 );
            $insurance_class->change_status( $order_id, $status, $unicue_code );
        }
...
//Pagination
if (!is_admin()):
    add_action('rcl_enqueue_scripts','add_on_order_pagination_js',10);
endif;

if(defined('DOING_AJAX') && DOING_AJAX){
    add_action('wp_ajax_nopriv_order_pagination', 'add_on_order_pagination'); // No privileges - т.е. гость
    add_action('wp_ajax_order_paginatio', 'add_on_order_pagination'); // любой залогиненный
}

function add_on_order_pagination_js(){
    rcl_enqueue_script( 'add-on-insurance-order-pagination', rcl_addon_url('js/add-on-insurance-pagination.js', __FILE__) );
}


function add_on_order_pagination() {
    ...
    $orders_list_data = $insurance->get_orders( $get_all_referals_id, $limit_from, $offset );

    //Отрисовываем заявки
    $render_orders_list = $insurance->render_orders( $orders_list_data );
    ...
    //Узнаем сколько всего заявок, ТЕСТОВЫЙ ИД 1
    $orders = $insurance->get_orders( $get_all_referals_id );
    $orders_count = count( $orders );
...
//Фильтр заявок
if (!is_admin()):
    add_action('rcl_enqueue_scripts','add_on_order_filter_js',10);
endif;

if(defined('DOING_AJAX') && DOING_AJAX){
    add_action('wp_ajax_nopriv_order_filter', 'add_on_order_filter'); // No privileges - т.е. гость
    add_action('wp_ajax_order_filter', 'add_on_order_filter'); // любой залогиненный
}

function add_on_order_filter_js(){
    rcl_enqueue_script( 'add-on-insurance-order-filter', rcl_addon_url('js/add-on-insurance-filter.js', __FILE__) );
}


function add_on_order_filter() {
...
//Excell
if (!is_admin()):
    add_action('rcl_enqueue_scripts','add_on_get_excell_order_js',10);
endif;

if(defined('DOING_AJAX') && DOING_AJAX){
    add_action('wp_ajax_nopriv_get_excell_order', 'add_on_get_excell_order'); // No privileges - т.е. гость
    add_action('wp_ajax_get_excell_order', 'add_on_get_excell_order'); // любой залогиненный
}

function add_on_get_excell_order_js(){
    rcl_enqueue_script( 'add-on-insurance-get_excell_order', rcl_addon_url('js/add-on-insurance-get-excell-order.js', __FILE__) );
}


function add_on_get_excell_order() {
...
function add_tab_insurance(){

    global $user_ID;
    
    $tab_data =	array(
        'id'=>'insurance',
        'name'=>'Мед. страхування',
        ...
        'content'=>array(
            array( //массив данных первой дочерней вкладки
                'callback' => array(
                    'name'=>'insurance_recall_block',//функция формирующая контент
                )
            )
        )
    );
...
function insurance_recall_block($user){
	add_thickbox();
	global $user_ID;
    global $wpdb;
